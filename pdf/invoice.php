...
$pdf->addText(50,34,6,' http://www.engineerinme.com');
$pdf->restoreState();
$pdf->closeObject();
...
$cost=$_SESSION['bookcost'];


$pdf->ezText("Customer invoice \n",30,array('justification'=>'centre'));
$pdf->ezText("Invoice number ::",20,array('justification'=>'centre'));
$pdf->ezText("Customer name  $abcd ",18,array('justification'=>'centre'));
$pdf->ezText(" book name :$name",18,array('justification'=>'centre'));
//there would be an error when this page is runned without logging in by creating a session in this line

$pdf->ezText("cost of book  $cost ",18,array('justification'=>'centre'));
$pdf->ezText("payment method cash on delivery  ",18,array('justification'=>'centre'));
$pdf->ezText("status of rquest :order processed  ",18,array('justification'=>'centre'));

//line for book name call by book id
$pdf->ezText("\n please download the invoice before closing the page invoice once deleted cannot be restored",18,array('justification'=>'centre'));
$pdf->ezText("<c:alink:http://www.books.engineerinme.com>Engineerinme book store </c:alink> ",18,array('justification'=>'centre'));
...
function ros_logo(&$pdf,$x,$y,$height,$wl=0,$wr=0){
  ...
  $pdf->setColor(0.6,0,0);
  $pdf->addText($x,$y-$th-$td,$ts*0.1,'http://www.engineerinme.com');
  $pdf->restoreState();
  return $height;
}
...
if (file_exists('ros.jpg')){
  ...
  $img = ImageCreatefromjpeg('http://www.books.engineerinme.com');
...
$pdf->ezNewPage();
$pdf->ezText("terms and conditions\n",26,array('justification'=>'centre'));
$xpos = 520;
